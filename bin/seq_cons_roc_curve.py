...
for index in cores:
    ...
    size.append(0)
    TE_set = set()
    with open(Arg[4]+str(index)+"_annotated.nodes", 'r') as f:
        for line in f:
            L = line[:-1].split('\t')
            if len(L) < 2 :
                continue
    ...
    #Update the lists
    ab_threshold_list.append(max_ed)
    TP_list.append(TP)
...
print("Last recall:", last_r, " true : ", len(true_TE_set), " TP_TE : ", TP_te, " Te seen : ", len(TE_seen))
...
for deg in sorted_keys:
    for index in dic_additional_nodes[deg]:
        TE_set = dic_index2TE[index].split("; ") if dic_index2TE[index] != "*" else []
        if len(TE_set) > 0:
            TP+=1
        else :
            FP+=1
...
#complete ex_deg_threshold_list with the additional nodes extended degree (which is the key of dic_additional_nodes) and complete the size list
c = np.array(ab_threshold_list)
#Complete size with 1
...
plt.savefig(Arg[5]+"all_nodes_of_comp.png", bbox_inches='tight')
plt.close()
