        ...
        with open(prefix + "_annotated_RM.nodes", 'w') as f_out:
            ...
            for line in f:
                ...
                if i in dic_index2consDfam:
                    ...
                    set_TE = set()
                    set_micro = set()
                    for (TE_el,as_el) in dic_index2consDfam[i]:
                        if TE_el[-2:]==")n" or (len(TE_el) > 5 and TE_el[-5:]=="-rich") or (len(TE_el) > 8 and TE_el[:8] in ["LSU-rRNA","SSU-rRNA"]) or (len(TE_el) > 5 and TE_el[:5]=="tRNA-") :
                            set_micro.add(TE_el)
                        else:
                            set_TE.add(TE_el)
                            if AS_dfam < as_el :
                                AS_dfam=as_el
                    str_consDfam = "; ".join([el for el in set_TE])
                    str_consRb = "; ".join([el for el in set_micro])
